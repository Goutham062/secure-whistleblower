...
export default function Home() {
  ...
  const endRideSafe = async () => { 
      if (rideReportId) {
          try {
              ...
              alert("Your previous SOS Alert has been cancelled.");
  ...
  const triggerSOS = async () => {
      ...
      } catch (e) { console.error("SOS Fail", e); }
  };
  ...
  // --- SUCCESS SCREEN ---
  if (submittedId) return <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6"><div className="bg-white p-10 rounded-3xl shadow-xl w-full max-w-md text-center"><div className="text-5xl mb-4">✅</div><h1 className="text-2xl font-bold text-slate-800">Report Logged</h1><div className="bg-slate-100 p-6 rounded-2xl mt-6 mb-6"><p className="text-slate-500 text-sm uppercase font-bold tracking-wider">Tracking ID</p><p className="text-4xl font-mono font-bold text-blue-600 mt-2">{submittedId}</p></div><button onClick={() => window.location.reload()} className="w-full bg-slate-900 text-white py-4 rounded-xl font-bold text-lg hover:scale-[1.02] transition">Done</button></div></div>;
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-20">
      ...
      {/* HEADER (FIXED: ADDED DASHBOARD BUTTON) */}
      <header className="bg-white shadow-sm sticky top-0 z-50 p-4">
        <div className="max-w-xl mx-auto flex justify-between items-center">
          ...
          <div className="flex gap-2">
             {/* DASHBOARD BUTTON IS HERE NOW */}
            <Link href="/dashboard">
              <button className="bg-blue-600 text-white text-xs px-3 py-2 rounded-lg font-bold shadow-md hover:bg-blue-700 transition">
                Dashboard
              </button>
            </Link>
