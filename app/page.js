...
const BAD_WORDS = ["stupid", "idiot", "fake", "prank", "test", "kill", "bomb"]; // Simple filter
...
export default function Home() {
  ...
  useEffect(() => {
    if (desc.length > 5) {
      ...
      setTimeout(() => setAiThinking(false), 800); // Fake delay for realism
    } else {
      setAiSuggestion("");
      setBlockSubmit(false);
    }
  }, [desc]); // Runs every time description changes
      ...
      <main className="max-w-xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-[2rem] shadow-2xl overflow-hidden border border-slate-100">
          ...
          <div className="p-6 md:p-8">
            {activeTab === 'report' && (
              <form onSubmit={handleReportSubmit} className="space-y-6">
                <div>
                  ...
                  <div className={`transition-all duration-300 ${aiSuggestion ? 'mb-2 p-2 bg-purple-50 text-purple-700 text-xs font-bold rounded-lg border border-purple-200' : 'h-0 overflow-hidden'}`}>
                      {aiSuggestion}
                  </div>
                ...
                <div>
                   <div className="flex justify-between items-end mb-2">
                      ...
                      {aiThinking && <span className="text-[10px] text-purple-600 font-bold animate-pulse">✨ AI Analyzing...</span>}
                      <button type="button" onClick={() => alert("Voice Recording Started...")} className="text-xs flex items-center gap-1.5 text-blue-600 bg-blue-50 px-3 py-1.5 rounded-full hover:bg-blue-100 font-bold">🎙️ Record</button>
                   </div>
                   <textarea placeholder={t.descPlace} value={desc} onChange={(e) => setDesc(e.target.value)} className={`w-full bg-slate-50 p-4 rounded-xl text-base h-32 border focus:ring-2 outline-none resize-none ${blockSubmit ? 'border-red-500 focus:ring-red-200' : 'border-slate-200 focus:ring-blue-500'}`} required />
                </div>
                ...
                <button type="submit" disabled={loadingReport || blockSubmit} className="w-full bg-slate-900 text-white py-5 rounded-2xl font-bold text-lg shadow-xl hover:scale-[1.02] active:scale-[0.98] transition disabled:opacity-50 disabled:bg-slate-400">{loadingReport ? "Encrypting..." : t.submitBtn}</button>
              </form>
            )}
            ...
            {/* RIDE GUARD, SAFE ZONES, FUNDING (SAME AS BEFORE, KEPT FOR COMPLETENESS) */}
            ...
            {activeTab === 'safe' && <div className="py-2"><div className="text-center mb-8"><h2 className="text-2xl font-bold text-slate-800">Safety Heatmap</h2></div>{loadingSafe ? <p className="text-center text-slate-400">Loading...</p> : (<div className="space-y-3 h-[500px] overflow-y-auto pr-2 custom-scrollbar">{areaSafety.map((item) => (<div key={item.name} className={`flex justify-between items-center p-5 rounded-2xl border transition-all hover:scale-[1.01] ${item.color}`}><div className="flex items-center gap-4"><div className={`w-4 h-4 rounded-full shadow-sm ${item.status === 'Safe' ? 'bg-green-500' : item.status.includes('Caution') ? 'bg-yellow-500' : 'bg-red-600 animate-pulse'}`}></div><span className="font-bold text-base text-slate-800">{item.name}</span></div><div className="text-right"><span className="block text-[10px] font-extrabold uppercase tracking-widest opacity-60">{item.status}</span><span className="text-sm font-bold">{item.count} Reports</span></div></div>))}</div>)}</div>}
            {activeTab === 'fund' && <div className="text-center py-6"><div className="text-6xl mb-6">🤝</div><h2 className="text-2xl font-bold text-slate-800 mb-3">Support the Mission</h2><p className="text-sm text-slate-500 mb-8 max-w-sm mx-auto">Anonymous donations to fund Guardians.</p><div className="bg-slate-100 p-6 rounded-3xl mb-8"><div className="flex justify-between text-xs font-bold text-slate-500 mb-2"><span>₹{currentFunds.toLocaleString()} Raised</span><span>Goal: ₹{MONTHLY_LIMIT.toLocaleString()}</span></div><div className="w-full bg-slate-300 rounded-full h-3 overflow-hidden"><div className="bg-green-500 h-3 rounded-full" style={{ width: `${(currentFunds / MONTHLY_LIMIT) * 100}%` }}></div></div></div><input type="number" placeholder="Enter Amount (₹)" value={donateAmount} onChange={(e) => setDonateAmount(e.target.value)} className="w-full bg-slate-50 text-center text-3xl font-bold p-6 rounded-2xl mb-4 border border-slate-200 outline-none focus:ring-2 focus:ring-green-500 text-green-700" /><button onClick={handleDonate} className="w-full bg-green-600 text-white py-5 rounded-2xl font-bold text-lg shadow-xl hover:scale-[1.02] transition">Donate Anonymously</button></div>}
